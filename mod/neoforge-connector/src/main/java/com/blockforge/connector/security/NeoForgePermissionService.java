...
public class NeoForgePermissionService {
    public PermissionCheckResult check(ServerPlayer player, BlockForgePermissionAction action) {
        ...
        if (!BlockForgeConfig.requirePermissions()) {
    ...
    public PermissionCheckResult check(ServerPlayer player, String node, int fallbackOpLevel) {
        ...
        boolean allowed = !BlockForgeConfig.requirePermissions() || hasPermission(player, node, fallbackOpLevel);
        return allowed
