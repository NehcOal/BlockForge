...
BlockForge 是一个本地优先的 Minecraft 风格方块建筑蓝图生成器。当前 v1.2.4
...
BlockForge v1.2.4-alpha.1 补齐 Fabric 和 Forge 的 Material Refund Undo Alpha parity。
NeoForge 仍然是当前功能最完整的 connector。Fabric 和 Forge 现在会在 build 时记录
生存模式材料事务，`/blockforge undo` 会恢复方块并返还已消耗材料；背包满时，溢出的
返还材料会掉落在玩家附近。
...
| Undo 材料返还 | 支持 | Alpha 支持 | Alpha 支持 |
...
现在会扣除生存模式材料，并在 `/blockforge undo` 时恢复方块和返还材料，但仍不支持
...
v1.2.4 起 Fabric 已包含 GUI Selector、Builder Wand、Ghost Preview、生存材料成本与
...
v1.2.4 起 Forge 已包含 GUI Selector、Builder Wand、Ghost Preview、生存材料成本与
Undo 材料返还 Alpha。本阶段仍不包含 BlockEntity NBT undo。NeoForge 仍然是当前功能
最完整、最稳定的 Connector 目标。
