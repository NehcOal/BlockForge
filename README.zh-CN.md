...
v4.0.0-beta.1 包含 Web 导入工作台、字段级校验报告、本地规则生成器、可选外部
...
BlockForge v4.0.0-beta.1 重点是把 v3 gameplay 系统推进到更适合实机和服务器测试的
Beta 候选，不新增 Web / AI 大功能。

Beta 重点：

- Builder Station common tick runtime：包含 loaded chunk、protection、material、
  quota、cooldown gate。
- Diagnostics 增加 active stations、active jobs、audit entries、quota denials、
  material network sources。
- 新增 Gameplay Beta 测试和 QA 报告文档。

仍是 partial：loader 集成的 station 真实世界放置、Material Cache GUI、Builder
Station GUI、audit persistence、dedicated server 实测。
...
BlockForge v4.0.0-beta.1 新增第一版 Build Planner。它会先把蓝图放置转换成
...
BlockForge v4.0.0-beta.1 开始把重点转向 Minecraft 内体验，目标是减少玩家
...
BlockForge v4.0.0-beta.1 新增可选 AI 生成链路，同时继续把本地规则生成器作为默认 fallback。
...
BlockForge v4.0.0-beta.1 是 Product Workbench Alpha：包含发布就绪整理、
