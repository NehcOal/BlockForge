...
BlockForge 是一个本地优先的 Minecraft 风格方块建筑蓝图生成器。当前 v1.2.0
包含 Web 蓝图生成、3D 预览、导出链路、NeoForge Connector 游戏内建造流程，以及
Fabric / Forge Connector Alpha Builder Wand 闭环。

[English README](./README.md) | [使用手册](./docs/USER_MANUAL.zh-CN.md)

## v1.2.0 Fabric / Forge Builder Wand Parity

BlockForge v1.2.0-alpha.1 补齐 Fabric 和 Forge 的 Builder Wand Alpha parity。
NeoForge 仍然是当前功能最完整的 connector。Fabric 和 Forge 现在可以选择蓝图、
设置旋转、获取 Builder Wand，并手持法杖右键方块生成建筑。
...
Fabric 和 Forge 的 Builder Wand 仍是 Alpha。它们目前不包含 GUI、Ghost Preview、
生存材料成本、Undo 材料返还或 BlockEntity NBT undo。

Fabric / Forge Builder Wand Alpha 流程：
...
手持 Builder Wand 右键方块生成建筑，然后执行 `/blockforge undo` 恢复方块。
...
v1.2.0 起 Fabric 已包含 Builder Wand Alpha。本阶段仍不包含 GUI、Ghost Preview、
...
v1.2.0 起 Forge 已包含 Builder Wand Alpha。本阶段仍不包含 GUI、Ghost Preview、
生存材料成本、背包扣除、材料返还或 BlockEntity NBT undo。NeoForge 仍然是当前功能
最完整、最稳定的 Connector 目标。
