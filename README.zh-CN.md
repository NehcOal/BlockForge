...
v4.4.0-beta.1 包含 Web 导入工作台、字段级校验报告、本地规则生成器、可选外部
...
BlockForge v4.4.0-beta.1 重点是把游戏内 GUI / Station Runtime 往真实 loader
接线推进，不新增 Web / AI 大功能。

Beta 重点：

- Material Cache quick-move routing：定义 cache / player inventory / hotbar 转移行为。
- Loader screen registration safety：避免 dedicated server 加载 client Screen 类。
- Station world mutation result：记录 placed / skipped / failed、undo snapshot 和 audit。
- Loader integration paths：统一 audit JSONL 与 diagnostics JSON 输出路径。
- Admin rollback integration result：记录 snapshot、world restore、material refund、audit、
  station update 状态。

仍是 partial：loader-specific Material Cache / Builder Station screens、station
真实世界放置、admin rollback world restore、dedicated server 实测。
...
BlockForge v4.4.0-beta.1 新增可选 AI 生成链路，同时继续把本地规则生成器作为默认 fallback。
...
BlockForge v4.4.0-beta.1 是 Product Workbench Alpha：包含发布就绪整理、
