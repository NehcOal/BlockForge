...
面向 Minecraft 1.21.1 的多加载器建筑蓝图工具。

BlockForge 是一个面向 Minecraft 1.21.1 的多加载器建筑蓝图工具。你可以在 Web
工作台设计或生成房屋蓝图，在游戏内预览，用 Builder Wand 建造，统计生存材料，
并安全撤销。
...
BlockForge v5.3.0-beta.1 冻结新的大玩法线，专注主流程收口：房屋蓝图、游戏内
预览/建造、材料安全、撤销、文档降噪和三端构建稳定。

Core Stable-ish Beta Path：

- Web Workbench / House Designer
- Blueprint JSON v1/v2 / Blueprint Pack / Sponge `.schem`
- GUI Selector / Blueprint Table / Builder Wand / Ghost Preview
- Build / Undo
- Survival material cost / material refund
- NeoForge 推荐主流程

Alpha / Partial：

- Material Cache GUI
- Builder Station / Material Link / Construction Core
- Station runtime / audit / quota / admin rollback
- Litematica import / external AI generation
...
- NPC / economy / story
- Marketplace / cloud sync 未实现

Minecraft 实机回归和 dedicated server smoke test 仍保持 pending，除非 QA 文档明确记录。

详见 [House Construction](./docs/HOUSE_CONSTRUCTION.md)、
[House Designer](./docs/HOUSE_DESIGNER.md) 和
[House Quality](./docs/HOUSE_QUALITY.md)。

## v5.1.0 Settlement Events + Project Chains Alpha

BlockForge v5.1.0-alpha.1 在聚落委托基础上新增事件、工程链、紧急修复、
稳定度压力和新的游戏内入口方块。
本轮不新增 Web / AI / 市场 / 云同步范围。

Alpha 重点：

- Event Board / 事件告示板
- Project Map / 工程地图
- Emergency Beacon / 紧急信标
- Supply Depot / 补给仓库
- Event Notice / 事件通知
- Project Charter / 工程章程
- Emergency Repair Kit / 紧急修复包
- Settlement Seal / 聚落印章
- common 纯 Java：settlement event、stability、project chain、emergency repair
  verification、event reward、persistence path 模型。
- Settlement Core / 聚落核心
- Contract Board / 委托板
- Reward Crate / 奖励箱
- Architect Desk / 建筑师书桌
- Architect Ledger / 建筑师账本
- Contract Token / 委托凭证
- Architect Seal / 建筑师印章
- common 纯 Java：settlement、contract、verification、reward、progression 模型。
- 12 个内置委托模板。
- NeoForge command scaffold：`/blockforge settlement ...`、
  `/blockforge contracts ...`、`/blockforge rewards ...`、`/blockforge architect ...`、
  `/blockforge events ...`、`/blockforge projects ...`、`/blockforge emergency ...`。

仍是 partial：事件/工程持久化、完整 Event Board / Project Map GUI、世界快照修复验证、
Minecraft 实机回归、dedicated server smoke test。

详见 [Settlements](./docs/SETTLEMENTS.md)、[Contracts](./docs/CONTRACTS.md)
...
和 [Emergency Repairs](./docs/EMERGENCY_REPAIRS.md)。

## v3.5.0 Builder Station + Multiplayer Server Rules Alpha

BlockForge v3.5.0-alpha.1 开始进入 Builder Station 和多人服务器规则开发线。

当前 Alpha 范围：

- Builder Station、Material Link、Construction Core 已在 NeoForge / Fabric /
  Forge 三端注册，并补齐原创占位资源。
- Builder Station job status / queue 纯数据模型，以及 command-driven
  status / step scaffold。
- Server rules：quota、cooldown、audit、project membership、材料来源策略。
- 纯 Java rule evaluator、server gameplay DTO 和资源覆盖测试。

仍是 partial：tick-based station 真实放置、持久化 station job、Material Cache
库存来源、多玩家冲突处理。

详见 [Builder Station](./docs/BUILDER_STATION.md) 和
[Material Network](./docs/MATERIAL_NETWORK.md)。

## v3.2.0 Construction Workflow + Build Planner Alpha

BlockForge v3.2.0-alpha.1 新增第一版 Build Planner。它会先把蓝图放置转换成
稳定、可预览、可校验的施工计划，再由 loader 侧决定是否执行真实放置。

重点能力：

- common 层新增 BuildPlan / BuildLayer / BuildStep 模型。
- 按 Y 层从低到高稳定分层。
- 纯逻辑校验重复坐标、缺失 palette 引用、世界高度越界。
- NeoForge / Fabric / Forge 都有每玩家 BuildPlan manager scaffold。
- NeoForge 提供 `/blockforge buildplan ...` command-driven Alpha。
- Repair plan 纯逻辑支持“只修缺失坐标”的计划生成。

当前限制：v3.2 的 step execution 是安全模拟批次，不会真实放置方块；真实放置仍使用
现有 direct build 和 Builder Wand BUILD 路径。

详见 [Build Planner](./docs/BUILD_PLANNER.md)。
...
BlockForge v5.3.0-beta.1 新增可选 AI 生成链路，同时继续把本地规则生成器作为默认 fallback。

AI Generation 重点能力：

- Local Rule Generator / 本地规则生成器不需要 API key。
- 可选 OpenAI provider 只通过服务端 API route 调用。
- 浏览器客户端不会读取或打包 `OPENAI_API_KEY`。
- 外部 AI 返回 AI Structure Plan v1，不直接信任最终 blocks。
- Structure Plan 必须先通过校验，再转换成 VoxelModel 和 Blueprint v2。
- AI 生成结果可继续使用现有 3D 预览和多格式导出。
- 外部 AI 请求会把提示词发送给所选 AI 服务，API 使用可能产生费用。

详见 [AI Generation](./docs/AI_GENERATION.md)。
...
| AI provider mock 测试 | 已通过 |
| NeoForge/Fabric/Forge Gradle build | 已通过 |
| 外部 AI API key 手动 smoke test | 未执行 |
| Minecraft 实机回归 | 待执行 |
| Browser visual QA | 待执行 |

## v1.9.0 Web Polish + 游戏内 GUI Alpha

BlockForge v1.9.0-alpha.1 继续 polish Web 导入、校验和本地规则生成工作台，
...
详见 [Web Workbench](./docs/WEB_WORKBENCH.md)、[GUI Search And Filters](./docs/GUI_SEARCH_AND_FILTERS.md) 和 [AI Generation](./docs/AI_GENERATION.md)。
...
## v1.9.0 渲染性能与截图导出 Alpha

BlockForge v1.9.0-alpha.1 增加 Web 3D 预览性能优化和截图导出。预览支持
`Auto`、`Mesh`、`Instanced` 三种 render mode；Auto 会在 300 个方块及以上按方块
类型切换到 InstancedMesh 渲染。

Web 渲染重点能力：

- 大模型 InstancedMesh 渲染。
- 显示方块数、唯一方块类型、render mode、draw groups 等轻量统计。
- 使用程序化 Minecraft-inspired 材质风格，不打包 Minecraft 原版贴图文件。
- 改进相机 fit，大模型不容易裁切，小模型不显得太小。
- `导出预览 PNG` 可用于 README、release notes、图库截图。

### 生成 README 截图

1. 打开 Web 应用并选择或生成模型。
2. 调整 3D 预览相机角度。
3. 点击 `Export Preview PNG`。
4. 需要用于 README 时，把图片放到 `public/screenshots/blockforge-hero.png`。

截图导出只读取当前预览 canvas，不上传数据。

## v1.6.0 Schematic Interop Alpha

BlockForge v5.3.0-beta.1 新增 Sponge `.schem` v3 互通。Web 可以导出和导入
...
| Workspace 导入/导出 | ✅ Alpha |
| Import Job Queue | ✅ Alpha |
| Import Worker | ✅ Alpha |
| Web 3D Instanced 渲染 | ✅ Alpha |
| Preview PNG 导出 | ✅ Alpha |
| Minecraft-inspired 程序化材质 | ✅ Alpha |
| Minecraft 原版材质包 | ❌ 不打包 |
...
- 当前大版本列车：`v2.5 AI Productization Alpha`。这不是 stable。
...
BlockForge v5.3.0-beta.1 是 Product Workbench Alpha：包含发布就绪整理、
