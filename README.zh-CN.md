...
BlockForge 是一个本地优先的 Minecraft 风格方块建筑蓝图生成器。当前 v1.2.1
包含 Web 蓝图生成、3D 预览、导出链路、NeoForge Connector 游戏内建造流程，以及
Fabric / Forge Connector Alpha GUI Selector + Builder Wand 闭环。

[English README](./README.md) | [使用手册](./docs/USER_MANUAL.zh-CN.md)

## v1.2.1 Fabric / Forge GUI Selector Parity

BlockForge v1.2.1-alpha.1 补齐 Fabric 和 Forge 的 GUI Selector Alpha parity。
NeoForge 仍然是当前功能最完整的 connector。Fabric 和 Forge 现在可以通过
`/blockforge gui` 或默认 `B` 键选择蓝图和旋转，然后手持 Builder Wand 右键方块生成建筑。
...
Fabric 和 Forge 的 GUI Selector 与 Builder Wand 仍是 Alpha。GUI 只负责选择蓝图和
旋转；它们目前仍不包含 Ghost Preview、生存材料成本、Undo 材料返还或 BlockEntity NBT undo。

Fabric / Forge GUI + Builder Wand Alpha 流程：
...
右键方块生成建筑，然后执行 `/blockforge undo` 恢复方块。
...
v1.2.1 起 Fabric 已包含 GUI Selector 与 Builder Wand Alpha。本阶段仍不包含 Ghost Preview、
...
v1.2.1 起 Forge 已包含 GUI Selector 与 Builder Wand Alpha。本阶段仍不包含 Ghost Preview、
生存材料成本、背包扣除、材料返还或 BlockEntity NBT undo。NeoForge 仍然是当前功能
最完整、最稳定的 Connector 目标。
