...
v4.1.0-beta.1 包含 Web 导入工作台、字段级校验报告、本地规则生成器、可选外部
...
BlockForge v4.1.0-beta.1 重点是补齐游戏内 GUI / Station Runtime 的安全公共层，
方便后续在三端逐步接真实菜单和服务端动作，不新增 Web / AI 大功能。

Beta 重点：

- Material Cache menu state、Builder Station status/action DTO，用于后续三端 GUI。
- Builder Station action validator：校验 create/start/pause/resume/step/cancel。
- Audit JSONL formatter、admin rollback decision、cooldown policy helper。
- Diagnostics 增加 active stations、active jobs、audit entries、quota denials、
  material network sources。
- 更新 Gameplay GUI、audit/rollback、Beta QA 文档。

仍是 partial：loader-specific Material Cache / Builder Station screens、station
真实世界放置、audit 文件写入、dedicated server 实测。

详见 [Gameplay Beta Testing](./docs/GAMEPLAY_BETA_TESTING.md) 和
...
BlockForge v4.1.0-beta.1 新增可选 AI 生成链路，同时继续把本地规则生成器作为默认 fallback。
...
MIT
# BlockForge v4.1.0-beta.1

BlockForge v4.1.0-beta.1 是 Product Workbench Alpha：包含发布就绪整理、
