...
BlockForge 是一个本地优先的 Minecraft 风格方块建筑蓝图生成器。当前 v1.2.2
包含 Web 蓝图生成、3D 预览、导出链路、NeoForge Connector 游戏内建造流程，以及
Fabric / Forge Connector Alpha GUI Selector + Builder Wand + Ghost Preview 闭环。

[English README](./README.md) | [使用手册](./docs/USER_MANUAL.zh-CN.md)

## v1.2.2 Fabric / Forge Ghost Preview Parity

BlockForge v1.2.2-alpha.1 补齐 Fabric 和 Forge 的 Ghost Preview Alpha parity。
NeoForge 仍然是当前功能最完整的 connector。Fabric 和 Forge 现在会在玩家手持
Builder Wand 且已有服务端确认的蓝图选择时显示客户端预览轮廓。
...
Fabric 和 Forge 的 GUI Selector、Builder Wand 与 Ghost Preview 仍是 Alpha。Ghost
Preview 只显示随旋转变化的包围盒和地面 footprint；它们目前仍不包含生存材料成本、
Undo 材料返还或 BlockEntity NBT undo。

Fabric / Forge GUI + Builder Wand + Ghost Preview Alpha 流程：
...
看向方块时应显示 Ghost Preview 轮廓，右键生成建筑，然后执行 `/blockforge undo` 恢复方块。
...
在世界中 build 蓝图，并按玩家撤销最近一次 Fabric build。

Fabric Alpha 复用 `mod/common` 中的蓝图解析、旋转和 build planning 数据。
v1.2.2 起 Fabric 已包含 GUI Selector、Builder Wand 与 Ghost Preview Alpha。本阶段仍不包含
生存材料成本、背包扣除、材料返还或 BlockEntity NBT undo。NeoForge 仍然是当前功能
...
dryrun 构建计划、在世界中 build 蓝图，并按玩家撤销最近一次 Forge build。

Forge Alpha 复用 `mod/common` 中的蓝图解析、旋转和 build planning 数据。
v1.2.2 起 Forge 已包含 GUI Selector、Builder Wand 与 Ghost Preview Alpha。本阶段仍不包含
生存材料成本、背包扣除、材料返还或 BlockEntity NBT undo。NeoForge 仍然是当前功能
最完整、最稳定的 Connector 目标。
