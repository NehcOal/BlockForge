...
BlockForge 是一个本地优先的 Minecraft 风格方块建筑蓝图生成器。当前 v1.1.2
包含 Web 蓝图生成、3D 预览、导出链路、NeoForge Connector 游戏内建造流程，以及
Fabric / Forge Connector Alpha 命令闭环。
...
本阶段不包含 GUI、Ghost Preview、Builder Wand、生存材料成本、背包扣除、材料返还
或 BlockEntity NBT undo。NeoForge 仍然是当前功能最完整、最稳定的 Connector 目标。
